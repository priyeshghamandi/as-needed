...
import { Avatar, Badge, Dot, Icon } from "@/components/primitives";
...
export function HeroDashboard() {
  ...
  const fillRate = 92 + (tick % 3);
  const openCount = 18 - (tick % 4);
  const ttFill = (11 + (tick % 5)).toFixed(0);
  const recovered = 7;
          ...
          <section className="col-span-10 bg-white px-5 py-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                <Badge tone="teal">
                  <Dot tone="green" pulse /> 14 coordinators online
                </Badge>
          ...
          <section className="col-span-5 bg-white px-5 py-4 border-t border-l border-ink-100">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <Badge tone="green">
                  <Dot tone="green" pulse /> {rns.filter((r) => r.dot === "green").length} live
                </Badge>
              </div>
            ...
            <ul className="divide-y divide-ink-100 rounded-lg border border-ink-100">
              {rns.map((p, i) => (
                <li key={i} className="px-3 py-2 flex items-center gap-3">
                  <Avatar initials={p.initials} tone={p.tone as "teal" | "amber" | "rose" | "sky" | "ink" | "violet"} size={26} />
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-mono text-ink-500">
                      <Dot tone={p.dot as "teal" | "green" | "amber" | "red" | "ink"} pulse={p.dot === "green"} />
                      <span>{p.status}</span>
                      <span>·</span>
                      <span>{p.miles}</span>
                      <span>·</span>
...
function FloatingChip({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={`flex items-center gap-2 bg-white shadow-lifted rounded-full pl-2 pr-3 h-8 text-[11px] font-mono ${className}`}
    >
      {children}
    </div>
  );
...
function NavItem({
  ...
  return (
    ...
    >
      ...
      {badge && (
        <span
          className={`text-[10px] font-mono px-1.5 h-4 inline-flex items-center rounded ${
            tone === "amber" ? "bg-amber-100 text-amber-800" : active ? "bg-paper/10 text-paper" : "bg-ink-100 text-ink-700"
          }`}
        >
          {badge}
        </span>
...
function CoordItem({
  ...
  return (
    <div className="flex items-center gap-2 px-1 py-1.5 rounded hover:bg-ink-50">
      <Avatar initials={initials} tone={tone as "teal" | "amber" | "rose" | "sky" | "ink" | "violet"} size={20} />
...
function RequestRow({
  ...
  r: { id: string; facility: string; shift: string; role: string; need: number; filled: number; urgency: string };
  odd: boolean;
  highlight: boolean;
}) {
  ...
  const stateLabel = { critical: "Critical", filled: "Filled", warning: "At risk", open: "Open" }[r.urgency as "critical" | "filled" | "warning" | "open"];
  return (
    <div
      className={`grid grid-cols-12 items-center px-3 py-2.5 text-[12px] ${odd ? "bg-paper/30" : "bg-white"} ${highlight ? "ring-1 ring-rose-200/60" : ""}`}
    >
      ...
      <div className="col-span-3 flex items-center gap-2">
        ...
        <span className="font-mono text-[11px] tabular-nums text-ink-700 w-10 text-right">
          {r.filled}/{r.need}
        </span>
      </div>
      <div className="col-span-1 text-right">
        <Badge tone={tone as "neutral" | "teal" | "green" | "amber" | "red" | "dark"}>
          {tone === "red" && <Dot tone="red" pulse />}
          {stateLabel}
        </Badge>
      </div>
    </div>
        ...
        {steps.map((s, i) => (
          <li key={i} className="flex flex-col">
            <div className="flex items-center gap-2">
              ...
              {i < steps.length - 1 && (
                <span className={`flex-1 h-px ${s.done && steps[i + 1].done ? "bg-teal-700" : "bg-ink-200"}`} />
              )}
            </div>
...
function ComplianceItem({ name, status, tone }: { name: string; status: string; tone: string }) {
  const tones: Record<string, string> = {
    amber: "text-amber-700",
    teal: "text-teal-700",
    green: "text-emerald-700",
  ...
  return (
    <div className="flex items-center gap-2 py-1.5 border-b last:border-0 border-ink-100">
      ...
      <Dot
        tone={tone === "red" ? "red" : tone === "amber" ? "amber" : tone === "green" ? "green" : "teal"}
        pulse={tone === "red"}
      />
    </div>
  );
