...
test.describe("Customer reject alternative", () => {
  test("ALT-E2E-003: customer rejects suggested alternative", async ({ browser }) => {
    ...
    await facility.getByRole("button", { name: /^reject$/i }).click();
    await facility.getByRole("button", { name: /reject suggested alternative/i }).click();
    await expect(facility.getByText(/rejected/i)).toBeVisible({ timeout: 15_000 });

    await facilityContext.close();
    await coordinatorContext.close();
  });

  test("ALT-E2E-006: agency withdraws pending alternative", async ({ browser }) => {
    const facilityContext = await browser.newContext();
    const coordinatorContext = await browser.newContext();
    const facility = await facilityContext.newPage();
    const coordinator = await coordinatorContext.newPage();
    ...
    await coordinator.getByRole("button", { name: /withdraw suggestion/i }).click();
    await expect(coordinator.getByText(/withdrawn/i)).toBeVisible({ timeout: 15_000 });
    await coordinator.reload();
    await expect(coordinator.getByRole("button", { name: /suggest alternative/i })).toBeVisible({
      timeout: 15_000,
    });

    await facility.goto(`/customer/requests/${requestId}`);
    await expect(facility.getByRole("heading", { name: /suggested alternative/i })).toHaveCount(0);
