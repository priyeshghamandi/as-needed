...
async function proposeAlternative(browser: import("@playwright/test").Browser) {
  ...
  return { facility, coordinator, requestId, facilityContext, coordinatorContext };
}

test.describe("Customer approve alternative", () => {
  test("ALT-E2E-002: customer approves suggested alternative", async ({ browser }) => {
    const { facility, requestId, facilityContext, coordinatorContext } =
      await proposeAlternative(browser);

    await facility.goto(`/customer/requests/${requestId}`);
    await expect(
      facility.getByRole("heading", { name: /suggested alternative/i }),
    ).toBeVisible({ timeout: 15_000 });
    await facility.getByRole("button", { name: /approve suggested alternative/i }).click();
