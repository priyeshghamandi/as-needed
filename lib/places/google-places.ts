...
function parseAddressComponents(components: AddressComponent[]): Pick<ServiceArea, "city" | "state" | "country"> {
  let city = "";
  let state = "";
  let country = "";
...
export async function autocompletePlaces(input: string): Promise<ServiceAreaSuggestion[]> {
  ...
  const out: ServiceAreaSuggestion[] = [];
...
export async function fetchPlaceDetails(placeId: string): Promise<ServiceArea | null> {
  const apiKey = getApiKey();
